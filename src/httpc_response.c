...
static struct {
	...
	{ 401, httpc_str_mk_string("401"),
	  httpc_str_mk_string("Unauthorized") },
	{ 403, httpc_str_mk_string("403"), httpc_str_mk_string("Forbidden") },
	{ 404, httpc_str_mk_string("404"), httpc_str_mk_string("Not Found") },
	{ 500, httpc_str_mk_string("500"),
	  httpc_str_mk_string("Internal Server Error") },
	{ 501, httpc_str_mk_string("501"),
	  httpc_str_mk_string("Not Implemented") },
...
} version_str_map[] = { { HTTP_1_1, httpc_str_mk_string("HTTP/1.1") },
						  ...
						  uint16_t status_code)
{
	...
	for (int i = 0;
	     i < sizeof(code_reason_map) / sizeof(code_reason_map[i]); i++) {
				      ...
				      const char *val)
{
	...
	rkv->value.str = res->header->str + start + strlen(key) +
			 sizeof(SPACE) + sizeof(COLON) + 1;
...
httpc_err_e httpc_response_finalize(httpc_response_t *res, uint16_t status_code)
{
	...
	if (!ct_found) {
		ret = httpc_response_set_header(res, "Content-Type",
	...
	size_t st_line_len =
		(version_str_map[0].version_str.len + sizeof(SPACE) +
		 res->status->status_code->len + sizeof(SPACE) +
		 res->status->reason->len + sizeof(CR) + sizeof(NL));
	...
	memcpy(res->rstr->str + version_str_map[0].version_str.len + 1 +
		       res->status->status_code->len,
	       " ", sizeof(SPACE));
	memcpy(res->rstr->str + version_str_map[0].version_str.len + 1 +
		       res->status->status_code->len + 1,
	       res->status->reason->str, res->status->reason->len);
	memcpy(res->rstr->str + version_str_map[0].version_str.len + 1 +
		       res->status->status_code->len + 1 +
		       res->status->reason->len,
	       "\r\n", 2);
	memcpy(res->rstr->str + st_line_len, res->header->str,
	       res->header->len);
	...
	if (res->data) {
		...
		if (NULL == tmp)
			return HTTPC_ERR_MEM_ALLOC;
		res->rstr->str = tmp;
		snprintf((char *)(res->rstr->str + st_line_len + header_len),
			 res->data->len, "%s\r\n", res->data->str);
	}
	return HTTPC_ERR_NONE;
}
