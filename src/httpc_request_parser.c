...
#define str13_cmp(s, c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12,    \
		  c13)                                                         \
	(s[0] == c0 && s[1] == c1 && s[2] == c2 && s[3] == c3 && s[4] == c4 && \
	 s[5] == c5 && s[6] == c6 && s[7] == c7 && s[8] == c8 && s[9] == c9 && \
	 s[10] == c10 && s[11] == c11 && s[12] == c12 && s[13] == c13)

#define str14_cmp(s, c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12,    \
		  c13, c14)                                                    \
	(s[0] == c0 && s[1] == c1 && s[2] == c2 && s[3] == c3 && s[4] == c4 && \
	 s[5] == c5 && s[6] == c6 && s[7] == c7 && s[8] == c8 && s[9] == c9 && \
	 s[10] == c10 && s[11] == c11 && s[12] == c12 && s[13] == c13 &&       \
...
static httpc_err_e __httpc_request_parser(httpc_str_t *r, httpc_method_e *m,
					  httpc_str_t **p, httpc_str_t **q,
					  httpc_version_e *v, httpc_str_t **h,
					  httpc_str_t **b);
...
{
	httpc_str_t rs = { .str = (unsigned char *)request,
			   .len = strlen(request) };
	// Memory allocation start
	*r = malloc(sizeof(**r));
	if (NULL == *r)
		return HTTPC_ERR_MEM_ALLOC;
	(*r)->request_line = malloc(sizeof(httpc_request_line_t));
	if (NULL == (*r)->request_line)
		return HTTPC_ERR_MEM_ALLOC;
	(*r)->request_line->path = NULL;
	(*r)->request_line->query = NULL;
	(*r)->request_line->kv_query = NULL;
	(*r)->header = NULL;
	(*r)->kv_header = NULL;
	(*r)->data = NULL;
	(*r)->json = NULL;
	// First level parsing
	httpc_err_e ret = __httpc_request_parser(
		&rs, &(*r)->request_line->method, &(*r)->request_line->path,
		&(*r)->request_line->query, &(*r)->request_line->version,
		&(*r)->header, &(*r)->data);
	if (ret != HTTPC_ERR_NONE)
		return ret;
	// Second level parsing
	ret = __httpc_parse_query((*r)->request_line->query,
				  &(*r)->request_line->kv_query);
	if (ret != HTTPC_ERR_NONE)
		return ret;
	ret = __httpc_parse_headers((*r)->header, &(*r)->kv_header);
	if (ret != HTTPC_ERR_NONE)
		return ret;
...
httpc_str_t *httpc_request_get_query(const httpc_request_t *r,
				     const char *query)
{
...
httpc_str_t *httpc_request_get_header(const httpc_request_t *r,
				      const char *header)
{
					  ...
					  httpc_str_t **b)
{
	...
	bool request_method_state_completed = false;
	bool request_path_state_completed = false;
	bool request_query_state_completed = false;
	bool request_version_state_completed = false;
			...
			case request_method:
				...
				case 3:
					if (str3_cmp(start, 'G', 'E', 'T',
						...
						request_method_state_completed =
							true;
						*m = GET;
						start = request + i + 1;
					} else if (str3_cmp(start, 'P', 'U',
							    'T', ' ')) {
						line_state = request_path;
						request_method_state_completed =
							true;
				...
				case 7:
					if (str7_cmp(start, 'H', 'T', 'T', 'P',
						...
						*v = HTTP_2;
						...
						start = request + i + 1;
						request_version_state_completed =
							true;
					}
					break;
				case 9:
	...
	if (!request_method_state_completed) {
		return HTTPC_ERR_INVALID_METHOD;
	} else if (!request_path_state_completed) {
		return HTTPC_ERR_INVALID_PATH;
	} else if (line_state == request_query &&
		   request_query_state_completed == false) {
		return HTTPC_ERR_INVALID_QUERY;
	} else if (!request_version_state_completed) {
		return HTTPC_ERR_INVALID_VERSION;
	} else if (!request_header_state_completed) {
		return HTTPC_ERR_INVALID_HEADER;
	} else if (state == request_body &&
		   request_body_state_completed == false) {
					 ...
					 httpc_key_value_pair_t **h)
{
	...
	while (i <= len) {
		...
		if ((hs->str[i] == CR && i + 1 < len && hs->str[i + 1] == NL) ||
			...
			switch (hh->key.len) {
			case 4:
				if (str4_cmp(hh->key.str, 'H', 'o', 's', 't',
					     COLON)) {
					if (!httpc_validate_header_host(
						    &hh->value))
						return HTTPC_ERR_INVALID_HEADER;
					host_found = true;
				}
				break;
			case 6:
				if (str6_cmp(hh->key.str, 'A', 'c', 'c', 'e',
					     'p', 't', COLON) &&
				    !httpc_validate_header_accept(&hh->value))
					return HTTPC_ERR_INVALID_HEADER;
				break;
			case 8:
				if (str8_cmp(hh->key.str, 'E', 'n', 'c', 'o',
					     'd', 'i', 'n', 'g', COLON) &&
				    !httpc_validate_header_encoding(&hh->value))
					return HTTPC_ERR_INVALID_HEADER;

				if (str8_cmp(hh->key.str, 'L', 'a', 'n', 'g',
					     'u', 'a', 'g', 'e', COLON) &&
				    !httpc_validate_header_language(&hh->value))
					return HTTPC_ERR_INVALID_HEADER;
				break;
			case 10:
				if (str10_cmp(hh->key.str, 'C', 'o', 'n', 'n',
					      'e', 'c', 't', 'i', 'o', 'n',
					      COLON) &&
				    !httpc_validate_header_connection(
					    &hh->value))
					return HTTPC_ERR_INVALID_HEADER;
				break;
			case 12:
				if (str12_cmp(hh->key.str, 'C', 'o', 'n', 't',
					      'e', 'n', 't', '-', 'T', 'y', 'p',
					      'e', COLON) &&
				    !httpc_validate_header_content_type(
					    &hh->value))
					return HTTPC_ERR_INVALID_HEADER;
				break;
			case 13:
				if (str13_cmp(hh->key.str, 'A', 'u', 't', 'h',
					      'o', 'r', 'i', 'z', 'a', 't', 'i',
					      'o', 'n', COLON) &&
