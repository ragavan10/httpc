	...
	for (int i = 0; i < num_a; i++) {
		...
		if (!httpc_str_cmp(a_seg[i], b_seg[i]) &&
			...
			if (NULL == _seg) {
				...
				httpc_request_path_seg_t *tmp = realloc(
					_seg, (*num + 1) * sizeof(*tmp));
				if (NULL == tmp) {
					err = HTTPC_ERR_MEM_ALLOC;
					goto ERROR;
				}
			...
			if (str3_cmp(s, 'i', 'n', 't', ':')) {
				httpc_str_t _val = { .str = &a_seg[i].str[0],
						     .len = a_seg[i].len };
				int int_val =
					__httpc_utils_is_valid_integer(&_val);
				if (int_val == -1) {
					err = HTTPC_ERR_INVALID_PATH;
					goto ERROR;
				}
	...
	while (i <= s->len) {
		if (s->str[i] == '/' || i == s->len) {
			...
			if (NULL == _tokens) {
				...
				httpc_str_t *tmp = realloc(
					_tokens, (*num + 1) * sizeof(*tmp));
				if (NULL == tmp) {
					err = HTTPC_ERR_MEM_ALLOC;
					goto ERROR;
				}
