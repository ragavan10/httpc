  ...
  if (NULL == cntx->endpoint) {
    ...
    for (uint16_t i = 0; i < cntx->num_endpoints; i++) {
      if (((endpoint_t *)(cntx->endpoint[i]))->m == m &&
          httpc_str_cmp(*((endpoint_t *)(cntx->endpoint[i]))->p, *p)) {
        return HTTPC_ERR_DUPLICATE_ENDPOINT;
      }
    }
    endpoint_t **tmp =
        realloc(cntx->endpoint, cntx->num_endpoints * sizeof(**tmp));
    if (NULL == tmp)
      return HTTPC_ERR_MEM_ALLOC;
...
void *httpc_process_raw_request(void *arg) {
  ...
  for (uint16_t i = 0; i < param->cntx->num_endpoints; i++) {
    if (r->request_line->method ==
        ...
        httpc_str_cmp(*r->request_line->path,
                      *((endpoint_t *)(param->cntx->endpoint[i]))->p)) {
      ret = ((endpoint_t *)(param->cntx->endpoint[i]))->efn(r, &res);
      enp_found = true;
      if (ret != HTTPC_ERR_NONE) {
        param->ret = ret;
        return NULL;
...
httpc_err_e httpc_init(httpc_t *cntx, uint16_t port, int num_workers) {
  ...
  if (bind(server_fd, (struct sockaddr *)address, sizeof(struct sockaddr_in)) <
      0) {
    printf("%s\n", strerror(errno));
...
#endif
  while (1) {
    if ((new_socket = accept(cntx->fd, (struct sockaddr *)cntx->address,
                             &addrlen)) < 0) {
      printf("%s\n", strerror(errno));
    ...
    httpc_err_e ret = httpc_thread_add_work(cntx->tp, httpc_process_raw_request,
                                            (void *)param);
#ifdef TEST
    count++;
    if (count > 2)
